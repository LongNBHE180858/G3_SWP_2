...
<head>
    ...
    <style>
        .eye-icon { cursor: pointer; margin-left: 5px; color: #888; }
        .eye-icon:hover { color: #000; }
        .table th, .table td { vertical-align: middle; }
        .action-icons i { margin: 0 6px; cursor: pointer; color: #1e88e5; }
        .action-icons i:hover { color: #1565c0; }
    </style>
</head>
<body class="bg-light">
            ...
            <nav>
                <ul class="pagination">
                    <c:if test="${pageIndex > 1}">
                        <li class="page-item">
                            <a class="page-link"
                               href="${pageContext.request.contextPath}/QuestionListServlet?page=${pageIndex-1}
                                     &search=${param.search}&filter=${param.filter}&pageSize=${param.pageSize}">
                                Previous
                            </a>
                        </li>
                    </c:if>
                    <c:forEach var="i" begin="1" end="${totalPage}">
                        <li class="page-item ${i==pageIndex?'active':''}">
                            <a class="page-link"
                               href="${pageContext.request.contextPath}/QuestionListServlet?page=${i}
                                     &search=${param.search}&filter=${param.filter}&pageSize=${param.pageSize}">
                                ${i}
                            </a>
                        </li>
